...
export default function CardLivros({livro}){
    ...
    useEffect(() => {
        ...
        const favorito = favoritos.find(f => f.id === livro.id)
    ...
    function desfavoritar(){
        setFavorito(false) 
        let favoritos = JSON.parse( localStorage.getItem("favoritos") ) || []
        const favoritosAtualizado = favoritos.filter(f => f.id !== livro.id )
    ...
    return (
        <div className="flex flex-col items-center justify-between gap-1 w-40 m-2 relative">
            ...
            <img className="rounded h-56" src={livro.poster} alt="capa do livro" />
